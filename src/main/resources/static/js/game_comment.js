...
// 解除逻辑删除函数
function unDeleted(gameCommentId, isDeleted) {
    // 发送ajax请求，修改用户逻辑删除字段信息
    $.post('/gameComment/deleteGameComment', {gameCommentId: gameCommentId, isDeleted:isDeleted}, function (res) {
        if (res.code == 200) {
            alert(res.msg);
            // 获取搜索框元素
            var searchKeyword = document.getElementById('searchKeyword');
...
// 关闭或开启删除确认图层函数
function closeOrOpenDeleteConfirm(gameCommentId, isDeleted) {
    // 获取删除确认图层元素
    ...
    yesBtn.onclick = function () {
        if (typeof gameCommentId === 'number') {
            // 单个删除
            console.log('后台单个删除！');
            $.post('/gameComment/deleteGameComment', {gameCommentId: gameCommentId, isDeleted:isDeleted}, function (res) {
                // console.log(res);
        ...
        } else if (gameCommentId.length <= 0) {
            // 没有选中要删除的行
            alert('错误！没有选中要删除的行！');
            // 关闭确认图层
            deleteConfirm.style.display = 'none'
            ...
            // console.log(gameCommentId)   // 看起来是一个数组对象
            // console.log(typeof gameCommentId)    // Object
            // 作为数组传递给后台时，后台接收到的东西为null，后台接收到的是空
            console.log('后台批量删除！');
            $.post('/gameComment/deleteGameCommentByIds', {gameCommentId: gameCommentId, isDeleted:isDeleted}, function (res) {
                if (res.code == 200) {
                    // 删除成功
                    alert(res.msg);
                    // 关闭删除确认图层
...
// 删除游戏函数
function deleteGameCommentById(gameCommentId, isDeleted) {
    // console.log(uid);
    // console.log(uid.length == false);
    closeOrOpenDeleteConfirm(gameCommentId, isDeleted);
}
...
function load(currentPage, username, isDeleted) {
    ...
    // 然后加载到页面上
    $.get('/gameComment/findGameCommentByUsernameWithPage', {currentPage: currentPage, username: username, isDeleted: isDeleted}, function (res) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (list[i].isDeleted == 0) {
                ...
                tr +=
                    ...
                    '                    <input type="checkbox" class="tbody_checkbox" name="gameCommentId" value="'+list[i].gameCommentId+'">\n' +
                    '                </td>\n' +
                    '                <td>'+list[i].gameCommentId+'</td>\n' +
                    '                <td>'+list[i].game.gname+'</td>\n' +
                    ...
                    '                    <a href="game_comment_update.html?gameCommentId='+list[i].gameCommentId+'&isDeleted='+list[i].isDeleted+'" class="edit_btn">修改</a>\n' +
                    '                    <button class="delete_btn" onclick="deleteGameCommentById('+list[i].gameCommentId+', '+1+')">删除</button>\n' +
                    '                </td>\n'
            } else {
                logic = '是';
                tr +=
                    ...
                    '                <td  class="game_manage">\n' +
                    '                    <a href="game_comment_update.html?gameCommentId='+list[i].gameCommentId+'&isDeleted='+list[i].isDeleted+'" class="edit_btn">修改</a>\n' +
                    '                    <button class="un_deleted" onclick="unDeleted(' + list[i].gameCommentId + ', '+0+')">解除删除</button>\n' +
                    '                </td>\n'
            }
...
// 4.给所有表格主体部分的选择按钮绑定事件，用for循环来给每一个选择框绑定事件
    ...
    deleteById.onclick = function () {
        ...
        closeOrOpenDeleteConfirm(arr, 1); // 这里有点神奇，居然将数组也能传递过去
    };
    ...
    if (isDeleted == 1){
        ...
        id_unDeleted_btn.onclick = function () {
            ...
            var gameCommentId = []; // 这里要声明一个空数组
            for (var i = 0; i < selectLis.length; i++) {
                if (selectLis[i].checked) {
                    // 当前选择框被选中了
                    // 由于属性获取出来的值是属于字符串，所以需要转换成Int数字类型
                    // console.log(selectLis[i].value);
                    gameCommentId.push(parseInt(selectLis[i].value));
                }
            }
            console.log(gameCommentId)
            // 到这里就可以将数组发送给后台了
            // 发起ajax请求批量解除逻辑删除
            $.post('/gameComment/deleteGameCommentByIds', {isDeleted: 0, gameCommentId: gameCommentId}, function (res) {
                console.log(res)
                if (res.code == 200){
                    // 批量解除删除成功，刷新当前页面
