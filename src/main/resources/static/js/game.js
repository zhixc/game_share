window.onload = function () {
    ...
    searchBtn.onclick = function () {
        ...
        var gameName = searchKeyword.value;
        load(null, gameName, isDeleted.value);
    }
    ...
    $.get('/user/checkLogin', {}, function (data) {
        ...
        if (data.code == 200) {
            ...
            if (data.extend.user.role.rid == 1){
                ...
                // 游戏管理员
                menuList += '<li><a href="game.html">游戏管理</a></li>' +
                    '<li><a href="game_comment.html">游戏评论管理</a></li>'
            }
            // 将lis放到菜单里面
...
// 解除逻辑删除函数
function unDeleted(gid) {
    // console.log(gid);
    // 发送ajax请求，修改用户逻辑删除字段信息
    $.post('/game/unDeleted', {gid: gid}, function (res) {
        if (res.code == 200) {
            alert(res.msg);
            // 获取搜索框元素
            var searchKeyword = document.getElementById('searchKeyword');
            // console.log(searchKeyword.value)
            load(1, searchKeyword.value, 1);
        }
    })
}

// 关闭或开启删除确认图层函数
function closeOrOpenDeleteConfirm(gid) {
    // 获取删除确认图层元素
    ...
    yesBtn.onclick = function () {
        ...
        if (typeof gid === 'number') {
            ...
            // console.log('后台单个删除！');
            // 发送ajax请求，根据 uid删除用户
            $.post('/game/deleteGameByGid', {gid: gid}, function (res) {
                // console.log(res);
                if (res.code == 200) {
                    // 删除成功
                    alert(res.msg);
            ...
            // 遍历数组gid，将其转为字符串，不知道什么原因，
            // 作为数组传递给后台时，后台接收到的东西为null
            // console.log('后台批量删除！');
            $.post('/game/deleteGameByGids', {gid: gid}, function (res) {
                if (res.code == 200) {
                    // 删除成功
                    alert(res.msg);
                    // 关闭删除确认图层
...
// 删除游戏函数
function deleteGameByGid(gid) {
    // console.log(uid);
    // console.log(uid.length == false);
    closeOrOpenDeleteConfirm(gid);
}

// 渲染游戏列表数据函数
function load(currentPage, gameName, isDeleted) {
    // 去掉全选框的勾选
    var id_checkbox_all = document.getElementById('id_checkbox_all')
    id_checkbox_all.checked = false
    // 发起 ajax请求，分页查询用户列表信息
    // 然后加载到页面上
    $.get('/game/findGameByName', {currentPage: currentPage, gameName: gameName, isDeleted: isDeleted}, function (res) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (list[i].isDeleted == 0) {
                ...
                tr += '' +
                    ...
                    '                    <input type="checkbox" class="tbody_checkbox" name="gid" value="' + list[i].gid + '">\n' +
                    '                </td>\n' +
                    '                <td>' + list[i].gid + '</td>\n' +
                    '                <td>' + list[i].gname + '</td>\n' + '<td>' + list[i].genres.ggName + '</td>' +
                    '                <td class="game_cover"><img src="../' + list[i].cover + '" alt=""></td>\n' +
                    '                <td>' + list[i].developer + '</td>\n' +
                    '                <td>' + list[i].publisher + '</td>\n' +
                    '                <td>' + list[i].gameScore + '</td>\n' +
                    '                <td>' + logic + '</td>\n' +
                    '                <td>' + list[i].releaseTime + '</td>\n' +
                    '                <td>' + list[i].gmtCreate + '</td>\n' +
                    '                <td>' + list[i].gmtModified + '</td>\n' +
                    '                <td class="game_review"><p>' + list[i].gameReview + '</p></td>\n' +
                    '<td class="add_game_video"><a href="game_video_upload.html?gid='+list[i].gid+'&isDeleted='+list[i].isDeleted+'" class="edit_btn">添加</a></td>'+
                    '                <!-- 操作列, 修改或删除 -->\n' +
                    '                <td class="game_manage">\n' +
                    '                    <a href="game_update.html?gid=' + list[i].gid + '&isDeleted='+list[i].isDeleted+'" class="edit_btn">修改</a>\n' +
                    '                    <button class="delete_btn" onclick="deleteGameByGid(' + list[i].gid + ')">删除</button>\n' +
                    '                </td>\n' +
                    '';
            } else {
                logic = '是';
                tr += '' +
                    ...
                    '                <td  class="game_manage">\n' +
                    '                    <a href="game_update.html?gid=' + list[i].gid + '&isDeleted='+list[i].isDeleted+'" class="edit_btn">修改</a>\n' +
                    '                    <button class="un_deleted" onclick="unDeleted(' + list[i].gid + ')">解除删除</button>\n' +
                    '                </td>\n' +
                    '';
            }
        ...
        // 调用分页条渲染函数进行渲染
        generatePageInfo(res, gameName, isDeleted);
...
// 4.给所有表格主体部分的选择按钮绑定事件，用for循环来给每一个选择框绑定事件
    ...
    if (isDeleted == 1){
        ...
        id_unDeleted_btn.onclick = function () {
            ...
            // 到这里就可以将数组发送给后台了
            // 发起ajax请求批量解除逻辑删除
            $.post('/game/unDeletedMultiple', {arr: arr}, function (res) {
                console.log(res)
                if (res.code == 200){
                    // 批量解除删除成功，刷新当前页面
...
var myGameName = ''
// 分页条数据渲染函数
function generatePageInfo(res, gameName, isDeleted) {
    myGameName = gameName
    // 分页条数据渲染
    ...
    var lis = '';
    var firstPage = '<li><a href="javascript:void(0);" onclick="javascript:load(1, myGameName, '+isDeleted+')">首页</a></li>';
    ...
    var beforePage = '<li><a onclick="javascript:load('+beforeNum+', myGameName, '+isDeleted+')">上一页</a></li>';
 ...
 */
    ...
    for (var i = start; i <= end; i++){
        ...
        if (res.extend.pb.currentPage == i){
            li = '<li onclick="javascript:load('+i+', myGameName, '+isDeleted+')"><a class="currPage">'+i+'</a></li>'
        }else {
            li = '<li onclick="javascript:load('+i+', myGameName, '+isDeleted+')"><a>'+i+'</a></li>'
        }
        lis += li;
    }
    ...
    var nextPage = '<li><a onclick="javascript:load('+nextNum+', myGameName, '+isDeleted+')">下一页</a></li>';
    var lastPage = '<li><a onclick="javascript:load('+res.extend.pb.totalPage+', myGameName, '+isDeleted+')">末页</a></li>';
